...
parser.add_argument('--video', help='Select one of those: MotorcycleChase, PolarBear, Elephants, Bike', required=True)

parser.add_argument('--modeldir', help='Folder the .tflite file is located in', default=None)
...
if not VIDEO in ["MotorcycleChase", "PolarBear", "Elephants", "Bike"]:
    print("Selecciona un video vàlid (MotorcycleChase, PolarBear, Elephants, Bike)")
    exit()
...
models = {
    "MotorcycleChase": "custom_model_lite0",
    "PolarBear": "custom_model_lite",
    "Elephants": "custom_model_lite",
    "Bike": "custom_model_lite"
}

MODEL_NAME = f"models/{models[VIDEO]}"

classNames = {
    "MotorcycleChase": "moto",
    "PolarBear": "oso",
    "Elephants": "elephant",
    "Bike": "moto"
}
...
for image_path in images:
    ...
    for i in range(len(scores)):
        ...
        if (object_name == classNames[VIDEO] and (scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            # Draw label
            label = '%s: %d%%' % (labelNames[VIDEO], int(scores[i]*100)) # Example: 'person: 72%'
